...
class ValueSimplifier(c_ast.NodeVisitor):
    ...
    def visit_ArrayDecl(self, node):
        if isinstance(node.dim, c_ast.BinaryOp):
            # Convert 
            node.dim = self.binary_to_constant(node.dim)

    # FuncCall args are of type ExprList
    def visit_ExprList(self, node):
        for i, expr in enumerate(node.exprs):
            node.exprs[i] = self.binary_to_constant(expr)

        
    def is_int(self, node):
        return isinstance(node, c_ast.Constant) and node.type == 'int'
    
    # Recursively Simplify constant expressions
    def binary_to_constant(self, node: c_ast.BinaryOp):
        if isinstance(node, c_ast.Constant):
            return node
        if isinstance(node.left, c_ast.BinaryOp):
            node.left = self.binary_to_constant(node.left)
        if isinstance(node.right, c_ast.BinaryOp):
            node.right = self.binary_to_constant(node.right)
